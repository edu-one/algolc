...
the index of the node that tail's `next` pointer is connected to (0-indexed). It is `-1`
...
Follow-up: solve with O(1) memory (Floyd's cycle detection), not O(n) space (hash set of
visited nodes). Builds on [0141-linked-list-cycle](../0141-linked-list-cycle/README.md) —
same detection phase, plus finding the cycle's entry point.
