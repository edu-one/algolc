...
TEST(LinkedListCycleII, Example1CycleToMiddle) {
    dv::lc::Solution s;
    std::vector<dv::lc::ListNode *> nodes;
    dv::lc::ListNode *head = make_list({3, 2, 0, -4}, 1, nodes);
    EXPECT_EQ(nodes[1], s.detectCycle(head));
    free_nodes(nodes);
}

TEST(LinkedListCycleII, Example2CycleToHead) {
    dv::lc::Solution s;
    std::vector<dv::lc::ListNode *> nodes;
    dv::lc::ListNode *head = make_list({1, 2}, 0, nodes);
    EXPECT_EQ(nodes[0], s.detectCycle(head));
    free_nodes(nodes);
}

TEST(LinkedListCycleII, Example3NoCycle) {
    dv::lc::Solution s;
    std::vector<dv::lc::ListNode *> nodes;
...
TEST(LinkedListCycleII, EmptyList) {
    dv::lc::Solution s;
    EXPECT_EQ(nullptr, s.detectCycle(nullptr));
}

TEST(LinkedListCycleII, NoCycleMultipleNodes) {
    dv::lc::Solution s;
    std::vector<dv::lc::ListNode *> nodes;
...
TEST(LinkedListCycleII, SelfLoop) {
    dv::lc::Solution s;
    std::vector<dv::lc::ListNode *> nodes;
...
TEST(LinkedListCycleII, CycleAtTail) {
    dv::lc::Solution s;
    std::vector<dv::lc::ListNode *> nodes;
    dv::lc::ListNode *head = make_list({1, 2, 3, 4}, 3, nodes);
    EXPECT_EQ(nodes[3], s.detectCycle(head));
    free_nodes(nodes);
}
